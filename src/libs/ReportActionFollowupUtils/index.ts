import {isAddCommentAction} from '@libs/ReportActionsUtils';
...
function hasUserMessageSinceQuestion(actions: ReportActions | undefined, questionReportActionID: string | undefined, agentAccountID: number): boolean {
    ...
    return Object.values(actions).some(
        (action) => isAddCommentAction(action) && action.actorAccountID !== agentAccountID && action.reportActionID !== questionReportActionID && action.created > questionCreated,
    );
}
